...
def attack_test(folder,model,target_label):
    ...
    print('TUAP path is:', str(param.trigger_gen.TUAP_wav_path))
    ...
    if (param.trigger_gen.trigger_pattern == 'clean_label'):
        f.write(attack_model_name + '------' + 'poison sample number:' + str(param.trigger_gen.max_sample) + ' trigger_pattern:' +str(param.trigger_gen.trigger_pattern) + ' TUAP:' +str(param.trigger_gen.TUAP_wav_path) + ' ASR: %.3f%%' % (100 * success_num / total_num) + '\n')
    else:
